...
RUN wget http://dl.fedoraproject.org/pub/epel/7/x86_64/Packages/h/hiredis-0.12.1-1.el7.x86_64.rpm -O /tmp/hiredis.rpm && \
    yum localinstall -y /tmp/hiredis.rpm && \
    rm -f /tmp/hiredis.rpm && \
    wget https://github.com/zmartzone/mod_auth_openidc/releases/download/v2.3.11/cjose-0.6.1.4-1.el7.x86_64.rpm -O /tmp/cjose.rpm && \
    yum localinstall -y /tmp/cjose.rpm && \
    rm -f /tmp/cjose.rpm && \
    wget https://github.com/zmartzone/mod_auth_openidc/releases/download/v2.3.11/mod_auth_openidc-2.3.11-1.el7.x86_64.rpm -O /tmp/mod_auth_openidc.rpm && \
